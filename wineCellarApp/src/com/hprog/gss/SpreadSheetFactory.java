...
import com.hprog.conn.HttpConHandler;
import com.hprog.conn.Response;
import com.hprog.sp.Entry;
import com.hprog.sp.Feed;
import com.hprog.sp.ParseFeed;
...
//	private String password;
	...
	private AndroidAuthenticator authenticator = null;
...
//	}
	 ...
	 * @return
	 */
	public static SpreadSheetFactory getInstance(AndroidAuthenticator authenticator){
...
//	}
	...
	private SpreadSheetFactory(AndroidAuthenticator authenticator){
...
//		Log.p(TAG, "AuthToken="+ authToken);
	...
	private String getSpreadSheetList(String title, boolean isTitleExact){
		...
		if(res.getResponseMessage().equalsIgnoreCase("token expired")){
			Log.p(TAG, "Token Has Expired...");
			authenticator.resetAuthToken(authToken);
			login(SPREADSHEET_API_SERVICE_NAME);
			getSpreadSheetList(title, isTitleExact);
		}
